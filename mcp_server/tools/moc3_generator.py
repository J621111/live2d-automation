...
class Moc3Generator:
    ...
    def _write_bones(self, handle: Any) -> None:
        ...
        for bone in bones:
            ...
            handle.write(
                struct.pack("<ff", float(scale.get("x", 1.0)), float(scale.get("y", 1.0)))
            )
...
"""
        ...
        validation = self._validate_export_bundle(model_name, output_path, files, model3_data, state)
    ...
    def _validate_export_bundle(
            ...
            if not moc_path.exists():
                ...
                with open(moc_path, "rb") as handle:
                    if handle.read(4) != b"MOC3":
                        errors.append("Exported .moc3 file does not contain the expected magic header")
        ...
        warnings.append("Exported bundle is a mock intermediate artifact and must be rebuilt in Cubism Editor.")
